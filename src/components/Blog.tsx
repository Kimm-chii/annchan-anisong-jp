          ...
          {siteData.blog.posts.map((post, index) => (
            ...
            >
              <div className="h-36 sm:h-48 overflow-hidden relative">
                ...
                />
                <div className="absolute top-2.5 left-2.5 sm:top-3 sm:left-3 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-bold text-pink-600 dark:text-purple-300 flex items-center gap-1 shadow-sm border border-pink-100 dark:border-slate-800">
              ...
              <div className="p-4 sm:p-5 flex flex-col flex-1">
                <h3 className="text-sm sm:text-base font-bold text-gray-800 dark:text-white mb-2 group-hover:text-pink-500 dark:group-hover:text-purple-300 transition-colors leading-snug">
                  <a href={post.link} target="_blank" rel="noopener noreferrer">{post.title}</a>
                </h3>
                <p className="text-gray-600 dark:text-slate-300 line-clamp-2 sm:line-clamp-3 mb-4 flex-1 text-xs sm:text-sm leading-relaxed transition-colors">
                  {post.excerpt}
                </p>
                <div className="pt-2 border-t border-pink-50 dark:border-slate-700/60 flex items-center justify-between">
                  <a href={post.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs font-bold text-pink-500 dark:text-purple-400 hover:text-pink-600 dark:hover:text-purple-300 transition-colors">
